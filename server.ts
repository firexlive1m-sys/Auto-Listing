...
`;
  ...
  app.post("/api/create-cashfree-order", async (req, res) => {
    try {
      ...
      if (!appId || !secretKey || appId.trim() === "" || secretKey.trim() === "" || appId === "YOUR_CASHFREE_APP_ID" || secretKey === "YOUR_CASHFREE_SECRET_KEY") {
        console.warn("Cashfree API keys are missing in environment variables (.env).");
        return res.status(400).json({
          error: "Cashfree API keys are not configured yet.",
          setupInstruction: "Please add CASHFREE_APP_ID and CASHFREE_SECRET_KEY to your environment variables in AI Studio settings or .env file to enable actual UPI payments."
        });
      }

      const orderId = "order_" + Math.random().toString(36).substring(2, 11) + "_" + Date.now();
      const url = cashfreeEnv === "production"
